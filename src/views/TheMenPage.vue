  ...
  <div>
    <div class="text-gray-700 grid md:grid-cols-5">
      ...
      <main class="px-16 py-6 bg-gray-100 md:col-span-4">
        <h1 class="font-bold">Men's Collections</h1>
        <div
          v-if="isLoading"
              ...
              >
                <router-link :to="{ path: `/shoe/${shoe.id}` }">
                  <img
                    :src="shoe.imageUrl"
                    alt="Shoe Image"
                    class="w-full h-32 sm:h-48 object-cover bg-gradient-to-r from-sky-400"
                  />
                  ...
                  <hr />
                  <p class="font-bold text-blue-600">Read More..</p>
                </router-link>
              </div>
            </div>
          </div>
  ...
  setup() {
    ...
    onMounted(() => {
      onSnapshot(shoesCollectionRef, (querySnapshot) => {
        ...
        basket.value = getShoes.filter(
          (getShoe) =>
            getShoe.gender === "Male" && getShoe.category === "Basketball"
        );
        running.value = getShoes.filter(
          (getShoe) =>
            getShoe.gender === "Male" && getShoe.category === "Running"
        );
        walking.value = getShoes.filter(
          (getShoe) =>
            getShoe.gender === "Male" && getShoe.category === "Walking"
        );
        training.value = getShoes.filter(
          (getShoe) =>
            getShoe.gender === "Male" && getShoe.category === "Training & Gym"
        );
