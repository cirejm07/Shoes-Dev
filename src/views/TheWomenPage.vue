<template>
  <div class="text-gray-700 grid md:grid-cols-5">
    ...
    <main class="px-16 py-6 bg-gray-100 md:col-span-4">
      <h1 class="font-bold">Women's Collections</h1>
      <div>
            ...
            >
              <router-link :to="{ path: `/shoe/${shoe.id}` }">
                <img
                  :src="shoe.imageUrl"
                  alt="Shoe Image"
                  class="w-full h-32 sm:h-48 object-cover bg-gradient-to-r from-fuchsia-200"
                />
    ...
    onMounted(() => {
      onSnapshot(shoesCollectionRef, (querySnapshot) => {
        ...
        basket.value = getShoes.filter(
          (getShoe) =>
            getShoe.gender === "Female" && getShoe.category === "Basketball"
        );
        running.value = getShoes.filter(
          (getShoe) =>
            getShoe.gender === "Female" && getShoe.category === "Running"
        );
        walking.value = getShoes.filter(
          (getShoe) =>
            getShoe.gender === "Female" && getShoe.category === "Walking"
        );
        training.value = getShoes.filter(
          (getShoe) =>
            getShoe.gender === "Female" && getShoe.category === "Training & Gym"
        );
      });
    });
