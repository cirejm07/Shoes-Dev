				...
				<div class="flex h-full bg-white rounded overflow-hidden shadow-lg">
					<a href="post.html" class="flex flex-wrap no-underline hover:no-underline">
						<div class="w-full md:w-2/3 rounded-t">	
							<img style="max-height: 500px;" src="https://source.unsplash.com/user/c_v_r" class="h-full w-full shadow">
						</div>
					...
					<div class="w-full md:w-1/3 p-6 flex flex-col flex-grow flex-shrink" v-for="shoe in menShoe" :key="shoe.id">
						<div class="flex-1 bg-white rounded-t rounded-b-none overflow-hidden shadow-lg">
							<a href="#" class="flex flex-wrap no-underline hover:no-underline">
								...
								<p class="w-full text-gray-600 text-xs md:text-sm px-6">{{ shoe.gender }}</p>
...
export default {
  ...
  querySnapshot.forEach((doc) => {
    ...
    return { name, menShoe, womenShoe, uniSexShoe , deleteHandler }
    }
}
</script>
